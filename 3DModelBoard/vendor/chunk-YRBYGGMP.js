import{a as e}from"./chunk-L5WUWJUF.js";var i="openpbrDielectricReflectance",t=`struct ReflectanceParams
...
`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=t);var n="openpbrGeometryInfo",c=`struct geometryInfoOutParams
...
{geometryInfoOutParams geoInfo=geometryInfo(normalW,viewDirectionW,roughness,geometricNormalW);geometryInfoAnisoOutParams outParams;outParams.NdotV=geoInfo.NdotV;outParams.NdotVUnclamped=geoInfo.NdotVUnclamped;outParams.environmentBrdf=geoInfo.environmentBrdf;outParams.horizonOcclusion=geoInfo.horizonOcclusion;outParams.anisotropy=vAnisotropy.b;vec3 anisotropyDirection=vec3(vAnisotropy.xy,0.);mat3 anisoTBN=mat3(normalize(TBN[0]),normalize(TBN[1]),normalize(TBN[2]));outParams.anisotropicTangent=normalize(anisoTBN*anisotropyDirection);outParams.anisotropicBitangent=normalize(cross(anisoTBN[2],outParams.anisotropicTangent));outParams.TBN=TBN;return outParams;}`;e.IncludesShadersStore[n]||(e.IncludesShadersStore[n]=c);var o="openpbrIblFunctions",s=`#ifdef REFLECTION
...
#endif
#endif
`;e.IncludesShadersStore[o]||(e.IncludesShadersStore[o]=s);var r="openpbrSubsurfaceLayerData",f=`float subsurface_weight=vSubsurfaceWeight;vec3 subsurface_color=vSubsurfaceColor.rgb;float subsurface_radius=vSubsurfaceRadius;vec3 subsurface_radius_scale=vSubsurfaceRadiusScale;float subsurface_scatter_anisotropy=clamp(vSubsurfaceScatterAnisotropy,-0.9999,0.9999);
